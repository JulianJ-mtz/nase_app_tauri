...
import { Variedad, VariedadData, obtenerVariedades, insertarVariedad, actualizarVariedad, eliminarVariedad } from "@/api/variedad_api";
...
export default function VariedadesPage() {
  ...
  const [currentVariedad, setCurrentVariedad] = useState<Variedad | null>(null);
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        <Dialog open={open} onOpenChange={handleOpenChange}>
          ...
          <DialogContent className="sm:max-w-[425px]">
            <DialogHeader>
              <DialogTitle>{editMode ? "Editar Variedad" : "Nueva Variedad"}</DialogTitle>
            </DialogHeader>
            <form onSubmit={handleSubmit} className="space-y-4 pt-4">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  type="number"
                  value={codigo ?? ""}
                  onChange={(e) => setCodigo(e.target.value ? Number(e.target.value) : undefined)}
                  required
                />
              </div>
                ...
                {variedades.length === 0 ? (
                  ...
                  variedades.map((variedad) => (
                    <TableRow key={variedad.id}>
                      ...
                      <TableCell>{variedad.codigo}</TableCell>
                      ...
                      <TableCell className="text-right">
                        <div className="flex justify-end space-x-2">
                          ...
                          <Button
                            variant="destructive"
                            size="sm"
                            onClick={() => handleDeleteVariedad(variedad.id)}
                          >
                            Eliminar
                          </Button>
                        </div>
