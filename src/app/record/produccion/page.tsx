...
import { Produccion, ProduccionData, obtenerProducciones, insertarProduccion, eliminarProduccion } from "@/api/produccion_api";
...
export default function ProduccionPage() {
  ...
  // Form state
  const [jornaleroId, setJornaleroId] = useState<number | undefined>(undefined);
  const [temporadaId, setTemporadaId] = useState<number | undefined>(undefined);
  const [variedadId, setVariedadId] = useState<number | undefined>(undefined);
  const [tipoUvaId, setTipoUvaId] = useState<number | undefined>(undefined);
  const [tipoEmpaqueId, setTipoEmpaqueId] = useState<number | undefined>(undefined);
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      const [prodData, jornData, tempData, varData, tipoUvaData, tipoEmpData, clienteData] = await Promise.all([
        obtenerProducciones(),
        obtenerJornaleros(),
        obtenerTemporadas(),
        obtenerVariedades(),
  ...
  const resetForm = () => {
    setJornaleroId(undefined);
    setVariedadId(undefined);
  ...
  const handleDeleteProduccion = async (id: number) => {
    if (confirm("¿Estás seguro de que deseas eliminar este registro de producción?")) {
      try {
        ...
        toast({
          title: "Éxito",
          description: "Registro de producción eliminado correctamente",
        });
        loadData();
      } catch (error) {
        ...
        toast({
          variant: "destructive",
          title: "Error",
          description: "No se pudo eliminar el registro de producción",
        });
      }
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!jornaleroId || !temporadaId || !variedadId || !tipoUvaId || !tipoEmpaqueId || !clienteId || !lote || !cantidad) {
      toast({
        variant: "destructive",
        title: "Error",
  ...
  const getJornaleroNombre = (id: number) => {
            ...
            <form onSubmit={handleSubmit} className="space-y-4 pt-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  <Label htmlFor="jornalero">Jornalero *</Label>
                  <Select 
                    value={jornaleroId?.toString()} 
                    onValueChange={(value) => setJornaleroId(Number(value))}
                  >
                    <SelectTrigger>
                    ...
                    <SelectContent>
                      {jornaleros.map((jornalero) => (
                        <SelectItem key={jornalero.id} value={jornalero.id.toString()}>
                          {jornalero.nombre}
                        </SelectItem>
                      ))}
                    </SelectContent>
                ...
                <div className="space-y-2">
                  ...
                  <Select 
                    ...
                    onValueChange={(value) => setTemporadaId(Number(value))}
                  >
                    <SelectTrigger>
                    ...
                    <SelectContent>
                      {temporadas.map((temporada) => (
                        <SelectItem key={temporada.id} value={temporada.id.toString()}>
                          Temporada {temporada.id} ({formatDate(temporada.fecha_inicial)})
                        </SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <Select 
                    value={variedadId?.toString()} 
                    onValueChange={(value) => setVariedadId(Number(value))}
                  >
                    <SelectTrigger>
                ...
                <div className="space-y-2">
                  ...
                  <Select 
                    ...
                    onValueChange={(value) => setTipoUvaId(Number(value))}
                  >
                    <SelectTrigger>
                    ...
                    <SelectContent>
                      {tiposUva.map((tipoUva) => (
                        <SelectItem key={tipoUva.id} value={tipoUva.id.toString()}>
                          {tipoUva.nombre}
                        </SelectItem>
                      ))}
                    </SelectContent>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <Select 
                    ...
                    onValueChange={(value) => setTipoEmpaqueId(Number(value))}
                  >
                    <SelectTrigger>
                    ...
                    <SelectContent>
                      {tiposEmpaque.map((tipoEmpaque) => (
                        <SelectItem key={tipoEmpaque.id} value={tipoEmpaque.id.toString()}>
                          {tipoEmpaque.nombre}
                        </SelectItem>
                      ))}
                    </SelectContent>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    min="0.01"
                    value={cantidad ?? ""}
                    onChange={(e) => setCantidad(e.target.value ? Number(e.target.value) : undefined)}
                    required
                  />
                </div>
              </div>
                ...
                {producciones.length === 0 ? (
                  ...
                  producciones.map((produccion) => (
                    <TableRow key={produccion.id}>
                      ...
                      <TableCell>{getJornaleroNombre(produccion.jornalero_id)}</TableCell>
                      <TableCell>{getVariedadNombre(produccion.variedad_id)}</TableCell>
                      <TableCell>{getTipoUvaNombre(produccion.tipo_uva_id)}</TableCell>
                      <TableCell>{getTipoEmpaqueNombre(produccion.tipo_empaque_id)}</TableCell>
                      <TableCell>{getClienteNombre(produccion.cliente_id)}</TableCell>
                      <TableCell>{produccion.lote}</TableCell>
                      <TableCell>{produccion.cantidad}</TableCell>
                      <TableCell className="text-right">
                        <Button
                          variant="destructive"
                          size="sm"
                          onClick={() => handleDeleteProduccion(produccion.id)}
                        >
                          Eliminar
                        </Button>
                      </TableCell>
